...
namespace Solution2 {
    int maxn = p_max; const int b_max = 1E4 + 100;
    LL G[p_max], Gsum[p_max], bp[b_max];
    map<LL, int> mem;

    void G_init() {
        G[0] = 0; G[1] = 1;
        FOR (i, 2, maxn) {
            if (minp[i] == i) G[i] = (bin(i, x, MOD) + MOD - bin(i / basep[i], x, MOD)) % MOD;
            else G[i] = G[minp[i]] * G[i / minp[i]] % MOD;
        }
        FOR (i, 1, maxn) Gsum[i] = (Gsum[i - 1] + G[i]) % MOD;
        FOR (i, 1, min(b_max, maxn)) bp[i] = bsum(x, i);
    }

    LL bsum_with_cache(LL n) {
        if (n < maxn && n < b_max) return bp[n];
        return bsum(x, n);
    }
