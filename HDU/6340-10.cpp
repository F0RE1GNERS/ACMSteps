...
int main() {
    ...
    while (T--) {
        ...
        FOR (k, 0, K + 2) t2[k] = bin(N, k + 1, MOD) * invf[k + 1] % MOD;
        NTT::conv(t1, t2, K + 10, G2);
        ...
        FOR (i, -1, K + 1) {
            G[i] = 0;
            LL post = B[i + 1] * invf[i + 1] % MOD * fac[K] % MOD;
            // FOR (j, 1, K + 2) FOR (k, 1, j + 2) {
            //     if (j - k != i) continue;
            //     // dbg(j, k, i);
            //     // LL d = A(K, j) * A(j, k) % MOD * bin(N, k, MOD) % MOD;
            //     // dbg(i, j, k);
            //     LL d = B[i + 1] * invf[i + 1] % MOD * fac[K] % MOD *
            //            B[K + 1 - j] % MOD * invf[K + 1 - j] % MOD * 
            ...
            // dbg(i, t, G[i], G2[K - i] * post % MOD, G2[K - i], G[i] * t % MOD);
            // (ans += G[i] * t) %= MOD;
